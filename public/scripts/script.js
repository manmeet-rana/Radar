$(document).ready(function () {
    ...
    function addTags(name) {
        $.ajax({
            ...
            console.log(data);
            var tags = data['key2'];
            $('#hashtags').html("<h4 class='text-center mb-4'>Popular hashtags</h4>");
            for (var i = 0; i < 15; ++i) {
    ...
    function addPosts(name) {
        $.ajax({
            ...
            console.log(posts);
            for (var i = 0; i < 1; i++) {
                $('#post1').append(((posts[i]['source'] == 'instagram') ? '<i class="fab fa-instagram"></i>' : '<i class="fab fa-twitter"></i>') + posts[i].post.substring(0, 75) + ' ' + posts[i].post.substring(75, 150) + '...');
                $('#post2').append(((posts[i + 1]['source'] == 'instagram') ? '<i class="fab fa-instagram"></i>' : '<i class="fab fa-twitter"></i>') + posts[i + 1].post.substring(0, 75) + ' ' + posts[i].post.substring(75, 150) + '...');
                $('#post3').append(((posts[i + 2]['source'] == 'instagram') ? '<i class="fab fa-instagram"></i>' : '<i class="fab fa-twitter"></i>') + posts[i + 2].post.substring(0, 75) + ' ' + posts[i].post.substring(75, 150) + '...');
                $('#post4').append(((posts[i + 3]['source'] == 'instagram') ? '<i class="fab fa-instagram"></i>' : '<i class="fab fa-twitter"></i>') + posts[i + 3].post.substring(0, 75) + ' ' + posts[i].post.substring(75, 150) + '...');

                $('#post1').next().attr('href', posts[i]['post_url']);
                $('#post2').next().attr('href', posts[i + 1]['post_url']);
                $('#post3').next().attr('href', posts[i + 2]['post_url']);
                $('#post4').next().attr('href', posts[i + 3]['post_url']);
    ...
    function getInstaDataForTwo(insta_username, insta_username2) {
        console.log("second username",insta_username,insta_username2);
        $.ajax({
            url: "http://insta-twi.herokuapp.com/insta/",
            method: "POST",
            data: { insta_user_name: insta_username }
        }).done(function (data) {
            console.log("in first parameter",insta_username2);
            $.ajax({
                url: "http://insta-twi.herokuapp.com/insta/",
                method: "POST",
                data: { insta_user_name: insta_username2 }
            }).done(function (data2) {
                console.log("in second parameter",data);
                console.log(data);
                console.log(data2);
                var i = 0, dateArr = [], followersArr = [];
    ...
    function getTwitterDataForTwo(twitter_username, twitter_username2) {
        $.ajax({
            ...
            $.ajax({
                ...
                console.log('twitter data 1:', data);
                console.log('twitter data 1:', data2);
                var i = 0, dateArr = [], followersArr = [];
                var j = 0, dateArr2 = [], followersArr2 = [];
